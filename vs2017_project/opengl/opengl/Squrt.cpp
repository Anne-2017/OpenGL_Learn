...
int main() {
   ...
   //顶点数组对象(Vertex Array Object, VAO)可以像顶点缓冲对象那样被绑定，任何随后的顶点属性调用都会储存在这个VAO中。
	...
	// 2. 把我们的顶点数组复制到一个顶点缓冲中，供OpenGL使用 //GL_ELEMENT_ARRAY_BUFFER 索引绘图必须使用这个缓存对象。只能使用一个索引，而公共的顶点会复制一份 类似obj
	glBindBuffer(GL_ARRAY_BUFFER, VBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
	// 3. 复制我们的索引数组到一个索引缓冲中，供OpenGL使用
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);//new
...
//第一个参数表示我们打算将其应用到所有的三角形的正面和背面，第二个参数告诉我们用线来绘制。之后的绘制调用会一直以线框模式绘制三角形，
//直到我们用glPolygonMode(GL_FRONT_AND_BACK, GL_FILL)将其设置回默认模式。
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0); //new
